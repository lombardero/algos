...
def CountQuickSort(input_data,start_idx,end_idx):
    if end_idx - start_idx > 1:
        ...
        if (end_idx - start_idx) % 2 == 0:
            middle = (end_idx - start_idx + 1)//2 + start_idx
        else:
            middle = (end_idx - start_idx + 1)//2 + start_idx - 1 
        last = end_idx
        three = [input_data[first],input_data[middle],input_data[last]]
        median_three = statistics.median(three)
        if median_three == input_data[first]:
            pivot_index = first
        elif median_three == input_data[middle]:
            pivot_index = middle
        else:
            pivot_index = last
